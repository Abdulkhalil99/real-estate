...
import { Property, PropertyFilters, Pagination as PT } from '@/types';
import PropertyGrid from '@/components/property/PropertyGrid';
import Pagination from '@/components/ui/Pagination';

const STATUS_OPTIONS  = [{ value: '', label: 'Any status' }, { value: 'FOR_SALE', label: 'For sale' }, { value: 'FOR_RENT', label: 'For rent' }, { value: 'SOLD', label: 'Sold' }, { value: 'RENTED', label: 'Rented' }];
const TYPE_OPTIONS    = [{ value: '', label: 'Any type' }, { value: 'HOUSE', label: 'House' }, { value: 'APARTMENT', label: 'Apartment' }, { value: 'CONDO', label: 'Condo' }, { value: 'LAND', label: 'Land' }, { value: 'COMMERCIAL', label: 'Commercial' }];
const SORT_OPTIONS    = [{ value: 'createdAt:desc', label: 'Newest first' }, { value: 'createdAt:asc', label: 'Oldest first' }, { value: 'price:asc', label: 'Price: low–high' }, { value: 'price:desc', label: 'Price: high–low' }, { value: 'area:desc', label: 'Largest first' }];
const BEDROOM_OPTIONS = [{ value: '', label: 'Any' }, { value: '1', label: '1+' }, { value: '2', label: '2+' }, { value: '3', label: '3+' }, { value: '4', label: '4+' }, { value: '5', label: '5+' }];
...
export default function PropertiesPage() {
  ...
  const fetchProperties = useCallback(async (f: PropertyFilters) => {
    setLoading(true);
    try {
      const result = await propertyApi.getAll(f);
  ...
  const activeCount = [filters.q, filters.status, filters.type, filters.city, filters.minPrice, filters.maxPrice, filters.bedrooms].filter(Boolean).length;
      ...
      <div style={{ background: 'white', borderBottom: '1px solid #e8edf2' }}>
        <div className="container py-8">
          ...
          <p style={{ color: '#7f8c8d', marginTop: '0.25rem' }}>
            {pagination ? `${pagination.total.toLocaleString()} properties found` : 'Searching...'}
          </p>
        </div>
      </div>
          ...
          <aside className={'lg:w-72 flex-shrink-0 ' + (filtersOpen ? 'block' : 'hidden lg:block')}>
            <div className="card p-5 sticky top-24">
              <div className="flex items-center justify-between mb-5">
                ...
                {activeCount > 0 && (
                  <button onClick={clearFilters} className="text-xs font-semibold flex items-center gap-1" style={{ color: '#3498db' }}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: '#3498db' }} />
                    <input type="text" className="input pl-9" placeholder="Villa, pool, sea view..."
                      value={filters.q || ''}
                ...
                <div>
                  ...
                  <input type="text" className="input" placeholder="Tunis, Sousse, Sfax..."
                    value={filters.city || ''}
                ...
                <div>
                  ...
                  <div className="relative">
                    <select className="input appearance-none" value={filters.status || ''}
                      onChange={(e) => updateFilter('status', e.target.value)}>
                      {STATUS_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                    </select>
                    <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: '#95a5a6' }} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select className="input appearance-none" value={filters.type || ''}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-5 gap-4">
              <button
                ...
                Filters
                {activeCount > 0 && (
                  <span className="w-5 h-5 rounded-full text-white text-xs flex items-center justify-center" style={{ background: '#3498db' }}>
                    {activeCount}
                  </span>
                )}
              </button>
              ...
              <div className="flex items-center gap-3 ml-auto">
                ...
                <div className="relative">
                  <select className="input appearance-none pr-8 py-2 text-sm" style={{ width: 180 }}
                    value={sortValue} onChange={(e) => handleSort(e.target.value)}>
                    {SORT_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                  </select>
                  <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: '#95a5a6' }} />
                </div>
              </div>
            </div>
            ...
            {/* Grid — 4 columns */}
            <PropertyGrid properties={properties} loading={loading} columns={4} />
