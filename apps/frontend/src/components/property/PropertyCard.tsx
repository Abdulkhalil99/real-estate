...
export default function PropertyCard({ property }: { property: Property }) {
  ...
  const isLoggedIn = !!authHelper.getUser();

  const handleFavorite = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();

    // اگر لاگین نیست به صفحه لاگین هدایت میشه
    if (!isLoggedIn) {
      sessionStorage.setItem("pending_favorite", property.id);
      window.location.href = "/auth/login";
      return;
    }

    const added = toggle(property.id);
    toast.success(added ? "Added to favorites ❤️" : "Removed from favorites");
  };
    ...
    <Link href={"/properties/" + property.id} className="group block">
      <div className="card card-hover">
        ...
        <div className="p-4">
          ...
          <h3
            className="font-semibold text-sm leading-snug mb-1.5 line-clamp-1"
            style={{ color: "#2c3e50" }}
          >
            {property.title}
          </h3>
          ...
          <div
            className="flex items-center gap-4 text-xs pb-3"
